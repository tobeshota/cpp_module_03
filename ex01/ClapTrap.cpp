...
/*   Updated: 2024/06/14 19:06:39 by toshota          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ClapTrap::ClapTrap(const std::string& name)
    : m_name(name),
      ...
      m_enegy_points(DEFAULT_ENEGY_POINTS),
      m_attack_damage(DEFAULT_ATTACK_DAMAGE) {
  std::cout << "(constructor)ClapTrap " << m_name << " has been created!"
            << std::endl;
}
...
ClapTrap::~ClapTrap() {
  std::cout << "(destructor)ClapTrap " << m_name << " has been destroyed!"
            << std::endl;
}
...
bool ClapTrap::can_move(void) const {
  if (m_hit_points <= 0) {
    ...
    std::cout << "ClapTrap " << m_name << "has no enegy points left!"
              << std::endl;
    return false;
  }
  return true;
...
void ClapTrap::beRepaired(unsigned int amount) {
  ...
  m_enegy_points--;
  m_hit_points += amount;
  std::cout << "ClapTrap " << m_name << " is repaired for " << amount
            << " points!" << std::endl;
}
